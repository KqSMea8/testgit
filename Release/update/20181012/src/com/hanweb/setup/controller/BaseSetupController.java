...
public class BaseSetupController {
	...
	public ModelAndView main() {
		...
		modelAndView.addObject("updateSetup", "db/updatesetup.do");
	...
	public ModelAndView baseSetup() {
		...
		String message = LicenceCheck.regMessage(BaseInfo.getRealPath(), appName + ".licence",
				appName);
		...
		modelAndView.addObject("machineCode", LicenceCheck.getMachineCode(
				BaseInfo.getRealPath(), appName + ".licence", appName));
	...
	public String save(BaseSetupForm setupForm) {
		...
		if (setupForm.getLicence() != null) {
			...
			if (StringUtil.equals(fileInfo.getFileType(), "licence")) {
				String licencePath = BaseInfo.getRealPath() + "/" + BaseInfo.getAppName()
						+ ".licence";
	...
	private void saveLogLevel(int logLevle) {
		String logpath = BaseInfo.getRealPath() + "/WEB-INF/classes/log4j.properties";
		Properties properties = new Properties(logpath);
		properties.setProperty("log4j.logger.com.hanweb", LogUtil.getLogLevelName(logLevle, 1));
