...
//		modelAndView.addObject("groupMenu", groupTreeJson);
	...
	public JsonResult submitAdd(GroupFormBean group) {
		...
		CurrentUser currentUser = UserSessionInfo.getCurrentUser();
		group.setSiteId(currentUser.getSiteId());
		try {
	...
	public JsonResult submitModify(GroupFormBean group) {
		...
		try {
			...
			if (isSuccess) {
				if (group.getPrevPid().intValue() != group.getPid().intValue()) {
	...
	public FileResource export(String ids, String pid) {
		...
		try {
			String filePath = groupService.exportGroup(ids, NumberUtil.getInt(pid), NumberUtil.getInt(siteId));
