...
public class MainFrameController {
	...
	public ModelAndView showMainPage() {
		...
		if(currentUser.isSysAdmin()){ 
			...
			if(haveRight(currentUser, "info")){
				try {
					menu.addMenuItem(MenuItem.getInstance("info", "infomanage", "消息管理",
							ControllerUtil.getAbsolutePath("/manager/menu/newsmenu_show.do"),
							ControllerUtil.getAbsolutePath("/manager/news/list.do?colId=0&colName=" 
									+ URLEncoder.encode("消息管理", "UTF-8")))
							.setPermission("info", null).setAllowed(false, true));
				} catch (UnsupportedEncodingException e) {
					logger.error("UTF-8加密失败", e);
				}
			...
			if(haveRight(currentUser, "collect")){
				menu.addMenuItem(MenuItem.getInstance("collect", "channelmanage", "收藏管理",
						ControllerUtil.getAbsolutePath("/manager/collect/list.do"))
						.setPermission("collect", null).setAllowed(false, true));
			}
			...
			if(!currentUser.isSysAdmin() && haveRight(currentUser, "matter")){		
				menu.addMenuItem(MenuItem.getInstance("matter", null, "素材管理",
						...
						setPermission("matter", null).setChannel("matter").setSeparator(true).setAllowed(false, true));
			}
			...
			/*if(haveRight(currentUser, "collect")){
				menu.addMenuItem(MenuItem.getInstance("collect", "sys", "收藏管理",
						ControllerUtil.getAbsolutePath("/manager/collect/list.do"))
						.setPermission("collect", null).setAllowed(false, true));
			}*/
			...
			if(currentUser.isSysAdmin()){
				menu.addMenuItem(MenuItem.getInstance("ditch", "sys", "群组管理",
						ControllerUtil.getAbsolutePath("/manager/qunzu/list.do"))
						.setPermission("ditch", null).setAllowed(false, true));
			}
	    }
		...
		menu.addMenuItem(MenuItem.getInstance("interface", null, "系统接口").setChannel("interfaceconfig")
				.setAllowed(true, true));
		
		menu.addMenuItem(MenuItem.getInstance("type", "interface", "接口分类",
				ControllerUtil.getAbsolutePath("/manager/typemenu/interfacetypemenu_show.do"),
				ControllerUtil.getAbsolutePath("/manager/interfaceType/list.do")).setAllowed(true, true));
		
		menu.addMenuItem(MenuItem.getInstance("info", "interface", "接口管理",
				ControllerUtil.getAbsolutePath("/manager/interfacemenu/interfacemenu_show.do"),
				ControllerUtil.getAbsolutePath("/manager/interface/list.do?typeId=0"))
				.setAllowed(true, true).setPermission("info", null));
		}
