...
<script type="text/javascript">
	...
	function addMembers(users, groups) {
		...
		ajaxSubmit("modify_submit.do?peoplelistid=${peoplelistid}&users=" + usersArray.join() + "&groups=" + groupsArray.join(), {
	...
	function removeMembers(ids) {
		...
		confirm("您确定要删除这" + ids.split(",").length + "条记录吗", function() {
			ajaxSubmit("remove.do?peoplelistid=${peoplelistid}&userIds="+usersArray.join()+"&groupIds="+groupsArray.join(), {
				success:function(result){
					if(result.success){
						location.reload();
					}else{
	...
	function cleanMembers() {
		ajaxSubmit('clean.do?peoplelistid=${peoplelistid}', {
			success:function(result){
				if(result.success){
					location.reload();
				}else{
