...
<script>
	...
	function loadUserData(currentId) {
		...
		$.ajax({
			...
			url : "../col/app_load.do",
			dataType : 'json',
			data : 'id=' + currentId +'&siteId=' + siteId,
	...
	$(function() {
		...
		function initUser(){
			var ids = '${appids}';
			var names = '${appNames}';
			var idAry = new Array(); //定义一数组 
			idAry = ids.split(","); //字符分割 

			var nameAry = new Array();
			nameAry = names.split(",");
			for (i=0;i<idAry.length ;i++ ) {
				if(idAry[i] && idAry[i].length>0){
					
						var id = idAry[i];
						var name = nameAry[i];
						var ic;
						var targetLi = '<li id="'+id+'" class="option user"><i class="iconfont">&#xf1006;</i><span class="optionname">' + name + '</span>';
						targetLi += ' <span class="ic-wrap">&lt;<span class="ic">' + name + '</span>&gt;</span>';
	...
	function ok() {
		...
		getDialog().dialog('options').callback(selectedUsers);
		closeDialog();
	}
</script>
<style>
				...
				<div id="usersselect" title="应用">
					<div style="height: 37px; padding-top: 5px; border-bottom: 1px dotted #CCC; background-color: #F9F9F9;">
						<div class="input-text" style="margin: auto; width: 370px;">
							<input id="user-search" class="searchtext" type="text" placeholder="应用" style="background-color: #FCFCFC; width: 345px; height: 22px; border: none; vertical-align: middle;" />
