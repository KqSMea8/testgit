...
public class ListMembersController implements GridViewDelegate {
	...
	@RequestMapping("list")
	public GridView list(GridView gridView, Integer peoplelistid, Integer memberType, String memberName) {
		if (memberType == null) {
			memberType = 0;
		}
		...
		gridView.setViewName("jmp/newspush/peoplelist/members_list");
		gridView.addQueryParam("peoplelistid", peoplelistid);
		createButton(gridView);
		createHead(gridView);
		if (StringUtil.isEmpty(memberName)) {
			memberName = gridView.getSearchText();
		}
		createBody(gridView, peoplelistid, memberName, memberType);
		gridView.setSearchPlaceholder("请输入成员名称");
		gridView.addObject("peoplelistid", peoplelistid);
	 ...
	 * @param gridView
	 */
	private void createBody(GridView gridView, Integer peoplelistid, String memberName, Integer memberType) {
		...
		gridViewSql
				...
				.setTable(
						com.hanweb.jmp.constant.Tables.PEOPLELISTRELATION + " r LEFT JOIN " + Tables.USER
		...
		where.append("r.peoplelistid = :peoplelistid");
		gridViewSql.addParam("peoplelistid", peoplelistid);
