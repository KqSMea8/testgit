...
public class UserMenuController {
	...
	public ModelAndView showUserMenu() {
		...
		List<Group> groupList = groupService.findChildGroupByIid(NumberUtil.getInt(rangeId), NumberUtil.getInt(siteId));
		...
		for (Group group : groupList) {
			...
			if(NumberUtil.getInt(Configs.getConfigs().getCreateAppType()) == 1 
					&& NumberUtil.getInt(Configs.getConfigs().getIsAutoCreateApp()) == 1) {
				tree.addNode(TreeNode.getInstance("appuser", "0", "运维用户",
				"/manager/appuser/list.do"));
			}
	...
	public String searchAsyncMenuWithUrl(String groupId, String isDisabled) {
		...
		List<Group> groupList = groupService.findChildGroupByIid(NumberUtil.getInt(groupId), NumberUtil.getInt(siteId));
