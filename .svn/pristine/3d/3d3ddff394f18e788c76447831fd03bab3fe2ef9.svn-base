...
<script>
	var orgType = '${orgType}';
	var useridsselected = '${usids}';
	var useridssele = {};
	var useridsnumber = 0;
	if(null != useridsselected && useridsselected.length > 0){
		useridssele = useridsselected.split(",");
		useridsnumber = useridssele.length;
	}
	var usersEnable = orgType.indexOf('s') == -1 ? false : true;
	...
	function checkisin(id){
		if(null != useridssele && useridssele.length > 0 && null != id && id.length > 0){
			for(p=0;p<useridssele.length;p++){
				if(id == useridssele[p]){
					return true;
				}
			}
		...
		$.ajax({
			...
			success : function(jsonData) {
				...
				$.each(groups, function(index, group) {
					...
					if(!usersEnable){
						groupLi += '<div class="checkbtn"' + disableBtn + '><i class="iconfont">&#xf3019;</i>添加</div>';
					}
					groupLi += '</li>';
	...
	$(function() {
		...
		if (usersEnable) {
			...
			// 检索机构和用户
			$('#user-search-btn').click(searchUsers);
			$('#user-search').bind('keydown', 'return', searchUsers);
		}
			...
			$.ajax({
				...
				success : function(jsonData) {
					...
					$.each(users, function(index, users) {
						...
						var userLi = '<li class="option user" id="'+id+'"><i class="iconfont">&#xf4005;</i><span class="optionname"'+disableTitle+'>' + text + '</span>';

						if (ic) {
							userLi += ' <span class="ic-wrap">&lt;<span class="ic">' + ic + '</span>&gt;</span>';
						}

						userLi += '<div class="checkbtn"' + disableBtn + '><i class="iconfont"></i>添加</div></li>';
		...
		function updateSize() {
			...
			if((userSize + useridsnumber - hasusersize) > 200){
				alert("最多只能选取200个用户!请移除" + (userSize + useridsnumber - hasusersize - 200) + "个用户！");
			}
	...
	function ok() {
		...
		if (usersEnable && !$.isEmptyObject(selectedUsers)) {
			noSelected = false;
		}
		...
		if ((userSize+ useridsnumber - hasusersize) > 200 ) {
			alert('用户最多只能选取200个，请移除' + (userSize + useridsnumber - hasusersize - 200) + "个用户！");
			return;
		}
		...
		getDialog().dialog('options').callback(selectedUsers, selectedGroups, selectedRoles,'${usids}');
		closeDialog();
	}
</script>
<style>
...
<body>
	...
	<div id="dialog-content" style="padding: 0; overflow: hidden;border: 1px solid #CCC;">
		<div id="orgselect-source" data-options="tabPosition:'left',headerWidth:100" class="easyui-tabs" style="width: 500px; height: 405px;">
			<c:if test="${fn:contains(orgType,'s')}">
				<div id="usersselect" title="用户">
					<div style="height: 37px; padding-top: 5px; border-bottom: 1px dotted #CCC; background-color: #F9F9F9;">
						<div class="input-text" style="margin: auto; width: 370px;">
							<input id="user-search" class="searchtext" type="text" placeholder="用户真实姓名名或拼音缩写，回车检索" style="background-color: #FCFCFC; width: 345px; height: 22px; border: none; vertical-align: middle;" />
