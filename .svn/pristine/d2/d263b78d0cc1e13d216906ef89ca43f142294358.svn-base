...
public class OutsideUserService {
	...
	public boolean add(OutsideUser outsideUser) throws OperationException {
		...
		outsideUser.setPassword(MD5.encodeByJavaSecurity(outsideUser.getPassword()));
		outsideUser.setRegtime(new Date());
		outsideUser.setPinYin(PinyinUtil.getHeadByString(outsideUser.getName()));
		if(user != null){
			outsideUser.setSiteId(user.getSiteId());
		}
	...
	public boolean modify(OutsideUser outsideUser) throws OperationException {
		...
		boolean isSuccess;
		outsideUser.setSiteId(outsideUserDAO.findByIid(outsideUser.getIid()).getSiteId());
		outsideUser.setClientType(outsideUserDAO.findByIid(outsideUser.getIid()).getClientType());
		outsideUser.setPinYin(PinyinUtil.getHeadByString(outsideUser.getName()));
		isSuccess = outsideUserDAO.update(outsideUser);
		if (isSuccess && StringUtils.isNotBlank(outsideUser.getPassword())) {
			outsideUser.setPassword(MD5.encodeByJavaSecurity(outsideUser.getPassword()));
			isSuccess = outsideUserDAO.updatePwd(outsideUser.getIid(), outsideUser.getPassword());
		}
