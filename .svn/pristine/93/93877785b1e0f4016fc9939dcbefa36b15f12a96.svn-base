...
public class OutsideUserDAO extends BaseJdbcDAO<Integer, OutsideUser> {
	...
	public OutsideUser findByIid(int iid) {
		String sql = "SELECT iid, name, loginname, headship, enable, mobile, siteid, clienttype,"
	...
	public OutsideUser findByLoginName(String loginName, Integer siteId) {
		String sql = getEntitySql() + " WHERE loginname = :loginName AND siteid=:siteid";
		Query query = createQuery(sql);
		query.addParameter("loginName", loginName);
	 ...
	 *         n - n个同名
	 */
	public int findIidByLoginName(String loginName, Integer siteId) {
		String sql = "SELECT COUNT(iid) FROM " + Tables.OUTSIDEUSER + 
			" WHERE loginname=:loginName AND siteid=:siteid";
		Query query = createQuery(sql);
		query.addParameter("loginName", loginName);
		query.addParameter("siteid", siteId);
		return this.queryForInteger(query);
