...
public class NewsMenuController {
	 ...
	 * @return    设定参数 .
	 */
	@RequestMapping(value = "newsmenu_show")
	public ModelAndView showDefaultTaskMenu(String colId, String siteId) {
		ModelAndView modelAndView = new ModelAndView("/jmp/newspush/news/news_menu");
		Tree tree = Tree.getInstance("colId", "colName");
		tree.addNode(TreeNode.getInstance("0", "", "消息管理", "/manager/news/list.do"));
	 ...
	 * @return    设定参数 .
	 */
	@RequestMapping("menuwithurlfornews_search")
	@ResponseBody
		...
		for (Col col : colList) {
			if(col.getType() == 2){
				...
				treeNode = TreeNode.getInstance(col.getIid() + "", colId, col.getName(),
						 " /manager/news/list.do", col.getIsParent(), false).addParam("colId",col.getIid())
