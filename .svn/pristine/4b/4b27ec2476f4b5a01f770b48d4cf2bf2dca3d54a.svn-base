...
<title>用户管理</title>
<h:head pagetype="dialog" multiselect="true" menu="true" tree="true" validity="true" ipass="true" checkpwd="true" tip="true" security="true" message="true" placeholder="true" iconfont="true" cookie="true"></h:head>
<script>
...
<body>
	<form action="${url }" method="post" id="oprform" name="oprform">
		...
		<input id="enc_password" type="hidden" name="user.pwd"/>
		<input id="enc_loginname" type="hidden" name="user.loginName"/>
				...
				<tr>
					...
					<c:choose>
						<c:when test="${url=='modify_submit.do'}">
							<td>&nbsp;</td>
							<td>
								<input type="text" id="loginname" maxlength="33" class="input-text" value="${formBean.user.loginName }" disabled="disabled" />
								&nbsp;
							</td>
						</c:when>
						<c:otherwise>
