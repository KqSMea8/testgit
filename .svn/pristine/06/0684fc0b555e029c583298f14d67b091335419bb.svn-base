...
<script type="text/javascript">
	function toolbarAction(action) {
		...
		case 'recycle':
			...
			page="${contextPath}/manager/matter/doc/recycle/list.do?classId=${classId}&name=${name}";
			location.href=page;
			break;
		...
		case 'remove':
			...
			openDialog("matter/doc/remove_show.do?ids=" + ids+"&idlength="+ids.split(",").length, 420, 270, {
				title : '删除附件'
			});
			break; 
		}
	...
	// 编辑
	function edit(iid) {
		openDialog('matter/doc/modify_show.do?iid=' + iid, 550, 250, {
			title : '附件编辑'  
		});
	}
...
<body>
	<div id="page-title">
	          附件分类 /  <span id="page-location">${name }</span>
	</div>
