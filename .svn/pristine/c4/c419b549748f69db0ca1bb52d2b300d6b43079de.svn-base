...
<script type="text/javascript">
	 ...
	 * 选择分类
	 */
	function selectdoc(classid){
		var dc = document.getElementById("lightAppName").value;
		if(dc==''){
			alert('请选择附件分类！');
			return false;
		}
		openDialog("matter/doc/add_show.do?classId="+classid, 980, 600, {
			title : '新增附件',
	...
	<form action="${url }" enctype="multipart/form-data" method="post" id="oprform">
		<div id="dialog-content">
		 	<table border="0" align="left" cellpadding="10" cellspacing="0" class="table">
				<tr id="apptr">
					...
					<td>
					<input type="text" name="lightAppName" id="lightAppName" readonly="readonly" class="input-text" value="${doccol.name}" /></td>
				</tr>
			</table>
		</div>
		<!--表单按钮区-->
        <div id="dialog-toolbar" >
			<div id="dialog-toolbar-panel">
				<input type="button" class="btn btn-primary" value="保存" onclick="selectdoc(classid)"/> 
