...
public class OprUserSelfController {
	...
	public ModelAndView showModifySelf() {
		...
		if(user.getGroupId() != null){
			groupName = groupService.findNameByIid(user.getGroupId());
		}
		...
		if(user.getUuid() != null){
			isSuperAdmin = user.getUuid().equals("bdf699cf97284db0a9524332f0648f22") ? true : false;
		}
	...
	public JsonResult submitModifyPassword(ModifyPasswordForm modifyPassword, HttpSession session,
		...
		try {
			...
			// 修改接口管理中的接口域名
			userService.modifyInterfaceDomain();
			
			// 修改社保、公积金等应用js的域名
			userService.modifyJsDomain(BaseInfo.getDomain());
