...
public class ListVideoRecycleController implements GridViewDelegate{
	...
	public GridView list(GridView gridView, Integer classId, Integer siteId,String name) {
		...
		gridView.setViewName("jmp/cms/matters/video/recycle_list");
	...
	private void createHead(GridView gridView) { 
		...
		gridView.addHead(Head.getInstance().setField("name").setTitle("音视频名称").setAlign("left").setResizable(true).setTip(true).setWidth(100)); 
	...
	private void createBody(GridView gridView, int classId, String searchText) { 
		...
		gridViewSql.addSelectField("b.iid").addSelectField("b.name")
		           .addSelectField("(select b.removetime from " +Tables.VIDEOCOL + " a where a.iid=b.classid)removetime ")
		           .setTable(Tables.VIDEO + " b");
