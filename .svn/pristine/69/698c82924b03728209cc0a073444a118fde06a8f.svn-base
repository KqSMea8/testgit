...
public class RightDAO extends BaseJdbcDAO<Integer, Right> {
	...
	public List<Right> findAll() {
		String sql = getEntitySql() + " WHERE codeid NOT IN (0012, 0020, 0021) " + " ORDER BY iid ";
		Query query = createQuery(sql);
		List<Right> rightList = this.queryForEntities(query);
		return rightList;
	}
	 ...
	 * @return
	 */
	public List<Right> findApp() {
		String sql = getEntitySql() + " WHERE codeid IN (0012, 0020, 0021)" + " ORDER BY iid ";
		Query query = createQuery(sql);
		List<Right> rightList = this.queryForEntities(query);
		return rightList;
	}
