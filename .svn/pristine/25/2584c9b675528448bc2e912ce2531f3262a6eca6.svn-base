...
public class ListPictureRecycleController implements GridViewDelegate{
	...
	public GridView list(GridView gridView, Integer classId, Integer siteId,String name) {
		...
		gridView.setViewName("jmp/cms/matters/picture/recycle_list");
	...
	private void createHead(GridView gridView) { 
		...
		gridView.addHead(Head.getInstance().setField("name").setTitle("图片名称").setAlign("left").setResizable(true).setTip(true).setWidth(100)); 
	...
	private void createBody(GridView gridView, int classId, String searchText) { 
		...
		gridViewSql.addSelectField("b.iid").addSelectField("b.name")
		           .addSelectField("(select b.removetime from " +Tables.PICTURECOL + " a where a.iid=b.classid)removetime ")
		           .setTable(Tables.PICTURE + " b");
