...
public class ListPictureColController implements GridViewDelegate{
	 ...
	 * @param request
	 * @param gridView
	 * @param pictureColName
	 * @return
	 */
	@RequestMapping("list")
	public GridView list(HttpServletRequest request, GridView gridView, String pictureColName) {
		gridView.setDelegate(this);
		gridView.setViewName("jmp/cms/matters/picture/picturecol_list");
		gridView.setSearchPlaceholder("请输入分类名称");
		gridView.setShowAdvSearch(false);
		gridView.addQueryParam("pictureColName", pictureColName);
	...
	private void createBody(GridView gridView, String searchName) {
		...
		gridViewSql.addSelectField("iid").addSelectField("name").addSelectField("createtime").setTable(Tables.PICTURECOL);
