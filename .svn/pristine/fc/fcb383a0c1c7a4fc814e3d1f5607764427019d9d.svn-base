...
<script type="text/javascript">
	function toolbarAction(action) {
		...
		case 'remove':
			...
			confirm("您确定要删除这" + ids.split(",").length + "条记录吗", function() {
				ajaxSubmit("remove.do?ids=" + ids + "&colname=" + getCheckedAttr("colname"), {success:function(result){
					if(result.success){
		...
		case 'add':
			openDialog('broketype/add_show.do', 550, 410, {
				title : '分类新增'
			});
			break;
			
		case 'sort':
			openDialog('broketype/sort_show.do', 550, 510, {
				title : '分类排序 '
			});
			break;
		}
	...
	function edit(iid, name) {
		openDialog('broketype/modify_show.do?iid=' + iid, 550, 410, {
			title : '分类编辑' 
		});
	}
</script>
...
<body>
	<div id="page-title"> 
	    <span id="page-location">报料分类</span>   
	</div>
	<div id="page-content">
