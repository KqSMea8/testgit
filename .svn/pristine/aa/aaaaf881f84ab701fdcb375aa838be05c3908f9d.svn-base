...
public class OprPictureController {
	...
	public ModelAndView showAdd(HttpServletRequest request, Integer classId) {
		ModelAndView modelAndView = new ModelAndView("jmp/cms/matters/picture/picture_add");
		Picture picture = new Picture();
		Configs configs = new Configs();
		String pictype = configs.getPicFileType();
		Integer picsize = configs.getPicFileSize();
		picture.setClassId(NumberUtil.getInt(classId));
		modelAndView.addObject("url", "add_submit.do");
		modelAndView.addObject("pictype", pictype);
		modelAndView.addObject("picsize", picsize);
		modelAndView.addObject("classId", classId);
		modelAndView.addObject("uploadUrl", BaseInfo.getContextPath() + "/manager/matter/picture/multifileupload.do");
	...
	public ModelAndView add() {
		ModelAndView modelAndView = new ModelAndView("jmp/cms/matters/picture/upload");
		modelAndView.addObject("uploadUrl", BaseInfo.getContextPath() + "/manager/matter/picture/fileupload.do");
		return modelAndView;
	}
	...
	public String upload(MultipartFile file) throws Exception {
		...
		if (fileName.contains(".")) {
			...
			newName = newName + "_source." + fileType.toLowerCase();
		}
	...
	@ResponseBody
	public String saveAdd(String filesJson,String fileinfo,PictureFormController picture, String classId) {
		String message = "";
		boolean isSuccess = false;
		CurrentUser currentUser = UserSessionInfo.getCurrentUser();
		picture.setSiteId(currentUser.getSiteId());
		picture.setIsremove(0);
		PictureCol pictureCol = pictureColService.findByIid(NumberUtil.getInt(classId));
		if(pictureCol!=null){
			picture.setPname(pictureCol.getName());
		}
		if (!StringUtil.equals("{}", filesJson) && StringUtil.isNotEmpty(filesJson)) {
			...
			for (String uuid : idSet) {
				...
				try {
					isSuccess = pictureService.add(picture, file);
	...
	public ModelAndView showModify(HttpServletRequest request, Integer iid) {
		ModelAndView modelAndView = new ModelAndView("jmp/cms/matters/picture/picture_opr");
		Picture picture = pictureService.findByIid(iid);
	...
	@ResponseBody
	public String submitModify(PictureFormController picture) {
		String message = "";
		boolean isSuccess = false;
		try {
			isSuccess = pictureService.modify(picture);
		} catch (OperationException e) {
			message = e.getMessage();
		}
		Script script = Script.getInstanceWithJsLib();
	...
	public ModelAndView showRemove(String ids, Integer idlength){ 
		ModelAndView modelAndView = new ModelAndView("jmp/cms/matters/picture/removepicture_opr"); 
	...
	@ResponseBody
	public JsonResult remove(PictureFormController picture, String ids,Integer siteId,Integer showList) {
		boolean isSuccess = false;
		...
		try {
			if(NumberUtil.getInt(showList)==1){
				isSuccess = pictureService.modifyIsRemove(1,ids, siteId);
			} else {
