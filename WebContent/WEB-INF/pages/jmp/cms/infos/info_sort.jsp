...
<script type="text/javascript">
	...
	function infodesc(){
		confirm("您确定要按照时间降序排序吗?", function() {
			ajaxSubmit("infodesc.do?colid=${colid}",{success:function(result){
					location.reload();
			}});
		});   
	}
...
<input type="hidden" name="colid" id="colid" value="${colid}"/>
		<div id="dialog-content">
			<table border="0" align="center" cellpadding="10" cellspacing="0" class="table">
				...
				<option value="${info.iid}" order="${info.orderid}">
					${info.title}
				</option>
				</c:forEach>
				</select></span>
			    ...
			    <input type="button" class="btn btn-success" value="降序" title="降序" onclick="infodesc()" style="margin-bottom:10px;width: 70px;text-align:center;" />
			    </td>
				</tr>
			</table>
		</div>
