	...
	function toolbarAction(action) {
		...
		case 'restore': 
			...
			confirm("您确定要还原这" + ids.split(",").length + "条记录吗", function() {
				ajaxSubmit("restore.do?ids=" + ids + "&siteId=${siteId}",{success:function(result){
					if(result.success){
						location.reload();
					}else{
						alert(result.message);
		...
		case 'back':
			...
			page="${contextPath}/manager/info/list.do?colId=${colId}"+"&fromurl="+url;
			location.href=page;
			break;
		} 
	}
...
<body>
	<div id="page-title">
	    <span>
		回收站   - <span id="page-title-text">${colName}</span> <i class="icon-caret-down"></i></span>
	</div>
	<div id="page-content">
		<h:grid></h:grid>
	</div>
