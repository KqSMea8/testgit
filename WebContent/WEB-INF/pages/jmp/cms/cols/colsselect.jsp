...
<script>
	...
	var colEnable = orgType.indexOf('c') == -1 ? false : true;

	var selectedGroups = {}; //已选机构
	var selectedCols = {}; //已选栏目
	...
	$(function() {
		...
		function searchGroup(e) {
			...
			$.ajax({
				url : '${contextPath}/manager/col/cols_search.do',
		...
		function updateSize() {
			...
			$.each(selectedCols, function(id, value) {
				if (value) {
					colSize++;
				}
			});
	...
	function ok() {
		...
		if (colEnable && !$.isEmptyObject(selectedGroups)) {
			noSelected = false;
		}
		...
		getDialog().dialog('options').callback(selectedGroups);
		closeDialog();
	}
</script>
<style>
			...
			<c:if test="${fn:contains(orgType,'g')}">
				<div id="groupselect" title="机构">
					...
					<div id="grouptreewrap" style="height: 358px; overflow: auto; position: relative;">
						<ul id="grouptree" class="easyui-tree selectsource batchselect" data-options="onCollapse:fitGroupTreeWrap,onExpand:fitGroupTreeWrap,onDblClick:toggleNode,onLoadSuccess:loadNodeSuccess" url="col/cols_load.do">
						</ul>
						<ul class="searchresult batchselect"></ul>
					</div>
				</div>
			</c:if>
			<c:if test="${fn:contains(orgType,'c')}">
				<div id="groupselect" title="栏目">
					<div style="height: 37px; padding-top: 5px; border-bottom: 1px dotted #CCC; background-color: #F9F9F9;">
						<div class="input-text" style="margin: auto; width: 370px;">
							<input id="group-search" class="searchtext" type="text" placeholder="栏目名缩写，回车检索" style="background-color: #FCFCFC; width: 345px; height: 22px; border: none; vertical-align: middle;" />
							<img id="group-search-btn" src="${contextPath}/ui/images/enter.png" title="点击此处检索" style="vertical-align: middle; cursor: pointer;">
						</div>
					</div>
					<div id="grouptreewrap" style="height: 358px; overflow: auto; position: relative;">
						<ul id="grouptree" class="easyui-tree selectsource batchselect" data-options="onCollapse:fitGroupTreeWrap,onExpand:fitGroupTreeWrap,onDblClick:toggleNode,onLoadSuccess:loadNodeSuccess" url="cols_load.do">
						</ul>
						<ul class="searchresult batchselect"></ul>
					</div>
				</div>
