...
<script type="text/javascript">
	function toolbarAction(action) {
		...
		case 'remove':
			...
			confirm('您确定要删除这' + ids.split(',').length + '条记录吗', function() {
				ajaxSubmit('../colfield_remove.do?ids='+ids, {success:function(result){
					if(result.success){ 
						location.reload();
					}else{
		...
		case 'add':  
			openDialog('site/colfield_show.do?siteId=${siteId}', 550, 480, {
				title : '展现类型新增'
			});
			break; 
	   }
	...
	function edit(iid, name) { 
		openDialog("site/modifycolfield_show.do?iid=" + iid, 550, 520, {
			title : '展现类型修改'
		});
	} 
</script>
