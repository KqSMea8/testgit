...
var callbacks={
    ...
    allCompleteCallback: function(){
    	if(typeof(allUploadCallback)!='undefined' && allUploadCallback instanceof Function){
    		allUploadCallback(imageUrls);
    	}
    },
    // 开始上传某个文件时的回调
...
function createImageUpLoad(opt){
	return createFlash(opt, callbacks);
}
function createFlash( opt, callbacks ) {
    ...
    option = extend( option, callbacks, false );
    return new baidu.flash.imageUploader( option );
}
